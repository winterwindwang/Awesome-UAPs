...
def get_dataset(data_name, system='linux', nb_images=500, input_size=224, only_train=True):
    if data_name == 'imagenet':
        ...
        if nb_images < 50000:
            ...
            sample_indices = np.random.permutation(range(train_dataset.__len__()))[:nb_images]
            train_dataset = Subset(train_dataset, sample_indices)
            print(f"{data_name}, data length: {len(sample_indices)}")
    elif data_name == 'voc':
        if "linux" in system:
            path = 'xxxx/dataset/VOC200712/VOCdevkit/VOC2012/JPEGImages/'
        elif "win" in system:
            path = "xxx/VOCPASCAL/VOCtrainval/VOCdevkit/VOC2007/JPEGImages/"
        train_transform = transforms.Compose([
            transforms.Resize(int(input_size * 1.143)),
            transforms.RandomCrop(input_size),
            transforms.ToTensor(),
        ...
        if "linux" in system:
            ...
            path = [r'xxx\ImageNet\train',
                    r'xxx\COCO2014\train2014',
                    r'xxx\VOCPASCAL\VOCtrainval\VOCdevkit\VOC2007\JPEGImages',
                    r'xxx\SUN397\train'
                    ]
        train_transform = transforms.Compose([
            transforms.Resize(256),
