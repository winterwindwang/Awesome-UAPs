...
def train(data_loader,
    ...
    while (iteration<num_iterations):
        ...
        target = torch.argmax(model(input), dim=1).detach()
        ...
        if model.module._get_name() == "Inception3":
            ...
            # modify for cosine-uap
            output = model(input)
            adv_input = input + torch.clamp(model.module.generator.uap.data, -epsilon, epsilon)
            output_uap = model(torch.clamp(adv_input, 0, 1))
            loss = criterion(output, output_uap).mean()
